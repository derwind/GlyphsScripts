...
def can_connect_as_remmen(fina_oval_path, init_oval_path, vertical_shift = UPM):
    ...
    INNER_ANGLE_THRESHOLD = 0.36 # almost 20 degrees
    ...
    # approximate inner angle
    abs_sin = abs(cross_product(fina_dir, init_dir))
    if abs_sin > INNER_ANGLE_THRESHOLD:
        return False

    d1 = dist_pt_and_line(fina_center, init_dir.y, -init_dir.x, -init_dir.y*init_center.x + init_dir.x*init_center.y)
