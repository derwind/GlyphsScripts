...
__doc__ = """
Find the terminal stroke and mark it.
"""
...
def terminal_stroke(path, terminal_edge):
    """
    find terminal stroke and calculate its center, its stem width and its direction, then return them
    """

    terminal_segments = []
    for segment in path.segments:
        for node in segment.points:
            if node.x == terminal_edge.x and node.y == terminal_edge.y:
    ...
    if approx_segment_length2(ts0) >= approx_segment_length2(ts1):
        return approx_segment_center(ts1), math.sqrt(approx_segment_length2(ts1)), approx_segment_direction(ts0, terminal_edge)
    else:
        return approx_segment_center(ts0), math.sqrt(approx_segment_length2(ts0)), approx_segment_direction(ts1, terminal_edge)

def is_terminal_path(path, terminal_edge):
    for node in path.points:
        if node.x == terminal_edge.x and node.y == terminal_edge.y:
            return True
    return False
...
def terminal_stroke_in_layer(layer):
    ...
    for path in layer.paths:
        if is_terminal_path(path, terminal_edge):
            center, stem_width, direction = terminal_stroke(path, terminal_edge)
            return center, stem_width, direction
    return None, None, None

def main():
    cur_layer = Glyphs.font.selectedLayers[0]
    center, stem_width, direction = terminal_stroke_in_layer(cur_layer)
    a = stem_width
    b = stem_width * 0.5
    ...
    cur_layer.paths.append(oval_path)
